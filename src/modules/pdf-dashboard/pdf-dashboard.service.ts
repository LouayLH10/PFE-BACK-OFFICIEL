...
export class PdfDashboardService {
  ...
  async generateDashboardPdf(
    ...
    const amount =
      this.formatTND(amountPaid);

    const balance =
      this.formatTND(balanceDue);
    ...
    const invoiceStats = {
      paid: filteredInvoices.filter(
        (invoice) =>
          invoice.status === 'PAID',
      ).length,

      sent: filteredInvoices.filter(
        (invoice) =>
          invoice.status === 'SENT',
      ).length,
    ...
    const orderStats = {
      ...
      recieved: filteredOrders.filter(
        (order) =>
